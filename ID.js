...
function checkTWId(id) {
    ...
    let ret = false ;
    let regex = /^[A-Z][12][0-9]{8}$/ ;  //身分證得限制條件
    let result = id.match(regex); //判斷有英文字母
    if (result ==null ) {
        let letters = 'ABCDEFGHJKLMNPQRSTUVXYWZIO'; 
        let c12 = id.substr(0,1); //取出第一碼的英文
        let n12 = letters.indexOf(c12) + 10 ; //判斷第一馬的值
        let n1 = parseInt(n12 / 10) ; //第一碼取出十位數
        let n2 = n12 % 10 ; // 第一碼取出個位數
